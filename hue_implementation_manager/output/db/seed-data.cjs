...
__export(data_exports, {
  ...
  DOCUMENTS: () => DOCUMENTS,
  EXPECTATION_AUDIENCES: () => EXPECTATION_AUDIENCES,
...
var STANDARD_DELIVERABLES = {
  pre: [
    ...
    { name: "Sign Master Stoploss Policy", rule: "10 days before effective date", note: "\u{1F534} Alert: Required to send ID cards" },
    { name: "Sign ACH Authorization document", rule: "10 days before effective date", note: "\u{1F534} Alert: Required to send ID cards" },
    { name: "Sign Stoploss Application and Disclosure Form", rule: "10 days before effective date", note: "\u{1F534} Alert: Required to send ID cards" },
...
var EXPECTATION_AUDIENCES = ["Employer", "Members", "Broker"];
...
function computeDueDate(effectiveDateStr, rule) {
...
var completedKeys = (group, indexes) => indexes.map((index) => deliverableKey(group, index, STANDARD_DELIVERABLES[group][index].name));
var DEMO_PROGRESS = {
  deliverableCompletion: Object.fromEntries(
    [...completedKeys("pre", [0, 1, 2, 3, 4, 5, 6, 7, 8]), ...completedKeys("post", [0])].map((key) => [key, true])
  ),
  // Denise walked through the general expectations with us before the effective date; each
  // entry is { by, at } — who reviewed it and the exact instant, same shape a live click writes.
  whatToExpect: Object.fromEntries(
    EXPECTATION_GROUPS[0].items.slice(0, 4).map((item, index) => [
      acknowledgementKey(EXPECTATION_GROUPS[0].title, item.title),
      { by: "Denise Whitfield", at: `2026-08-04T19:1${index}:00Z` }
    ])
  )
};
